...
import visitor
from mel_ast import LiteralNode, AssignNode, StmtListNode, FuncDeclNode, IdentNode, ReturnOpNode, VarsDeclNode, \
    BinOpNode, TypeConvertNode, CallNode, IfNode, WhileNode, ForNode, TernaryNode, CombEqNode, UnarOpNode
from code_gen_base import CodeLabel, CodeGenerator, find_vars_decls, DEFAULT_TYPE_VALUES
from semantic_base import BaseType, ScopeType, BinOp, TypeDesc, CombEqOp, UnarOp
...
class JbcCodeGenerator(CodeGenerator):
    ...
    def jbc_gen(self, node: VarsDeclNode) -> None:
    ...
    def jbc_gen(self, node: CombEqNode) -> None:
        if node.op == CombEqOp.ADD_EQ:
            node.arg1.jbc_gen(self)
            node.arg2.jbc_gen(self)
            if node.arg1.node_type == TypeDesc.STR:
                self.add(
                    f'invokestatic {RUNTIME_CLASS_NAME}#concat({JBC_TYPE_NAMES[BaseType.STR]}, {JBC_TYPE_NAMES[BaseType.STR]})')
            else:
                self.add(f'{JBC_TYPE_PREFIXES[node.arg1.node_type.base_type]}add')
    ...
    def jbc_gen(self, node: UnarOpNode) -> None:
        if node.op == UnarOp.INC:
            node.arg.jbc_gen(self)
            self.add('iconst_1')
            self.add('iadd')
            var = node.arg
            if var.node_ident.scope == ScopeType.LOCAL:
                self.add('istore', var.node_ident.index)
            elif var.node_ident.scope == ScopeType.PARAM:
                self.add('istore', var.node_ident.index)
            elif var.node_ident.scope in (ScopeType.GLOBAL, ScopeType.GLOBAL_LOCAL):
                self.add(
                    f'getstatic {self.class_name}#{JBC_TYPE_NAMES[var.node_ident.type.base_type]} _gv{var.node_ident.index}')
                self.add('iconst_1')
                self.add('iadd')
                self.add(
                    f'putstatic {self.class_name}#{JBC_TYPE_NAMES[var.node_ident.type.base_type]} _gv{var.node_ident.index}')
    ...
    def jbc_gen(self, node: ReturnOpNode) -> None:
        node.arg.jbc_gen(self)
        self.add(f'{JBC_TYPE_PREFIXES[node.arg.node_type.base_type]}return')
    ...
    def jbc_gen(self, node: TernaryNode) -> None:
        true_label = CodeLabel()
        end_label = CodeLabel()

        node.cond.jbc_gen(self)
        self.add('ifeq', true_label)

        # Generate code for true case
        node.true_expr.jbc_gen(self)
        self.add('goto', end_label)

        # Label for false case
        self.add(true_label)
        node.false_expr.jbc_gen(self)
    ...
    def jbc_gen(self, func: FuncDeclNode) -> None:
        var_offset = 0
        params = ''
    ...
    def gen_program(self, prog: StmtListNode):
        ...
        self.add('{')
        self.add('invokestatic ' + self.class_name + '#void main()')
