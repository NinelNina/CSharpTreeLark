...
from mel_ast import AstNode, VarsDeclNode
from semantic_base import BaseType
...
def find_vars_decls(node: AstNode) -> List[VarsDeclNode]:
    vars_nodes: List[VarsDeclNode] = []

    def find(node: AstNode) -> None:
        for n in (node.childs or []):
            if isinstance(n, VarsDeclNode):
                vars_nodes.append(n)
            else:
                find(n)
